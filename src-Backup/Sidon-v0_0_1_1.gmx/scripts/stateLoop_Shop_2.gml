/// stateLoop_Shop_2()
{
    ...
    if (abs(_u) + abs(_v) > 0) {
        if ( theTimer == 0 ) {
            ...
            var thePrice = DBQ_LookItems( theChoice[4] + theScroll[4], PRICE );

            if ( peek > -1 ) {
                var theMax = min( 99 - ds_list_find_value( Storage[QUANTITY], ds_list_find_index( Storage[INDEX], theChoice[4] + theScroll[4] ) ), floor( Storage[GOLD] / thePrice ) );
                //var theMax = min( 99, floor( Storage[GOLD] / thePrice ) - ds_list_find_value( Storage[QUANTITY], theChoice[4] + theScroll[4] ) );
                //var theMax = 99 - max(ds_list_find_value(Storage[QUANTITY], ds_list_find_index(Storage[INDEX], theChoice[4] + theScroll[4])), floor(Storage[GOLD] / thePrice));
            } else {
