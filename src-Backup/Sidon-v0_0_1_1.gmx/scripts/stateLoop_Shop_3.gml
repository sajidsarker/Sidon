/// stateLoop_Shop_3()
{
    ...
    if (myInput.keyA
     or myInput.keyStart) {
        ...
        myInput.keyA = false;
        if (theScroll[6] == 1) {
            BufferPrompt("0_Purchased [" + DBQ_LookItems(theChoice[4] + theScroll[4], NAME) + "] x" + string(theScroll[5]) + ".");
            AddGold(DBQ_LookItems(theChoice[4] + theScroll[4], PRICE) * -theScroll[5]);
