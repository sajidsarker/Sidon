/// stateLoop_Inventory_3()
{
    ...
    /*
    if (ds_list_size(Storage[ITEM_ID]) < 1) {
        theChoice[3] = 0;
        theScroll[3] = 0;
    }
    */
    ...
    if (myInput.keyA
     or myInput.keyStart) {
        ...
        if (ds_list_size(Storage[ITEM_ID]) > 0) {
            if (theScroll[6] == 1) {
                BufferPrompt("0_Sold [" + DBQ_LookItems(ds_list_find_value(Storage[ITEM_ID], theChoice[3] + theScroll[3]), ITEM_NAME) + "] x" + string(theScroll[5]) + ".");
                AddGold(DBQ_LookItems(ds_list_find_value(Storage[ITEM_ID], theChoice[3] + theScroll[3]), ITEM_PRICE) * theScroll[5]);
                AddStorage(ds_list_find_value(Storage[ITEM_ID], theChoice[3] + theScroll[3]), -theScroll[5]);
                theScroll[5] = 1;
                theScroll[6] = 0;
                if (theChoice[3] + theScroll[3] == ds_list_size(Storage[ITEM_ID])) {
                    if (theChoice[3] > 0) {
