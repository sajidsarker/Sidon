/// stateLoop_Shop_1()
{
    ...
    if (myInput.keyStart
     or myInput.keyA) {
        ...
        if (peek > -1) {
            var theMax = 99 - ds_list_find_value(Storage[QUANTITY], ds_list_find_index(Storage[INDEX], theChoice[4] + theScroll[4]));
        } else {
            var theMax = 99;
        }
        ...
        if (thePrice > Storage[GOLD]) {
            BufferPrompt("0_You do not have enough Gold to make a purchase.");
            DisplayPrompt();
        }
        if (theMax <= 0) {
